...
describe('property intelligence MVP', () => {
  it('runs a deterministic analysis for a complete input', () => {
    ...
    expect(report).toContain('Melhor direção estratégica')
    expect(report).toContain('## Como a Análise Foi Constituída')
    expect(report).toContain('Esta secção explica a mecânica do dossiê')
  ...
  it('serializes a JSON report with the same business signals as the markdown report', () => {
    ...
    expect(markdown).toContain('## Como a Análise Foi Constituída')
  ...
  it('emits markdown-only output through the CLI runner', () => {
    ...
    try {
      ...
      expect(output).toContain('## Como a Análise Foi Constituída')
