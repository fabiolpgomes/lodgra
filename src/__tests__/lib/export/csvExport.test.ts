import { buildForecastCSV, generateForecastCSV } from '@/lib/export/csvExport';
...
describe('buildForecastCSV', () => {
  it('formats forecast values with the provided currency', () => {
    ...
    const csv = buildForecastCSV(mockData, 'My Property', '2026-07-22', '2026-10-20', 'BRL');

    expect(csv).toContain('Projected Revenue (BRL)');
    expect(csv).toMatch(/R\$\s*2.500,00|R\$\s*2500,00/);
    expect(csv).toMatch(/R\$\s*85,00/);
