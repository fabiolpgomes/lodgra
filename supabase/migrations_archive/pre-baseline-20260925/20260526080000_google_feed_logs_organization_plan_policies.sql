-- Align Google feed log access with organization-level subscription plans.
-- The SaaS plan is stored on organizations.plan/subscription_plan, not properties.tier.
...
CREATE POLICY google_feed_logs_select_policy ON public.google_feed_logs
  ...
  USING (
    EXISTS (
      ...
      AND COALESCE(o.subscription_plan, o.plan) IN ('premium', 'professional', 'business', 'pro')
    )
  );
