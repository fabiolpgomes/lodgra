...
DO $$ BEGIN
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Admin can view all profiles' AND tablename = 'user_profiles') THEN
    CREATE POLICY "Admin can view all profiles" ON public.user_profiles FOR SELECT USING (
