...
-- Contract v2 owns its canonical fields. Legacy fields remain mandatory only
-- for contract v1 so a v2 row never carries invented legacy semantics.
ALTER TABLE public.regras_repasse
  ALTER COLUMN base_comissao DROP NOT NULL,
  ALTER COLUMN base_comissao DROP DEFAULT,
  ALTER COLUMN taxa_limpeza_para DROP NOT NULL,
  ALTER COLUMN taxa_limpeza_para DROP DEFAULT,
  ALTER COLUMN comissao_ota_por_conta DROP NOT NULL,
  ALTER COLUMN comissao_ota_por_conta DROP DEFAULT,
  ADD CONSTRAINT regras_repasse_contract_payload_check CHECK (
    (contract_version = 1
      AND base_comissao IS NOT NULL
      AND taxa_limpeza_para IS NOT NULL
      AND comissao_ota_por_conta IS NOT NULL)
    OR
    (contract_version = 2
      AND base_comissao IS NULL
      AND taxa_limpeza_para IS NULL
      AND comissao_ota_por_conta IS NULL)
  );
...
COMMENT ON FUNCTION lodgra_private.prevent_unversioned_v2_rule_closure() IS
  'Fail-closed gate: v2 rules may only be closed in the same transaction as the reviewed v2 replacement RPC.';
...
COMMENT ON FUNCTION public.replace_property_payout_rule_v2(
  uuid, uuid, date, public.tipo_comissao_repasse, numeric, numeric,
  text, text, text, boolean, smallint, jsonb, text
) IS 'Substitui atomicamente uma regra por uma política financeira v2 completa, tenant-scoped e versionada.';
