...
ALTER TABLE public.email_extractions
  ADD COLUMN IF NOT EXISTS extraction_version text NOT NULL
    DEFAULT 'email-reservation-extraction/v1',
  ADD COLUMN IF NOT EXISTS extraction_model text;

COMMENT ON COLUMN public.email_extractions.extraction_version IS
  'Versioned extraction contract used to produce this row.';
COMMENT ON COLUMN public.email_extractions.extraction_model IS
  'Provider model identifier used to produce this row.';

ALTER TABLE public.calendar_blocks
  ADD COLUMN IF NOT EXISTS created_at timestamptz NOT NULL DEFAULT now(),
  ADD COLUMN IF NOT EXISTS updated_at timestamptz NOT NULL DEFAULT now();

-- A feed UID is a single availability fact. Remove only redundant copies,
-- keeping the most recently updated row, then make concurrent upserts safe.
WITH ranked_blocks AS (
  SELECT id,
         row_number() OVER (
           PARTITION BY organization_id, property_listing_id, external_uid
...
USING ranked_blocks duplicate
WHERE cb.id = duplicate.id AND duplicate.position > 1;

CREATE UNIQUE INDEX IF NOT EXISTS idx_calendar_blocks_feed_uid_unique
  ON public.calendar_blocks (organization_id, property_listing_id, external_uid);

CREATE OR REPLACE FUNCTION public.upsert_calendar_event_audit(
  p_organization_id uuid,
  p_property_id uuid,
  p_property_listing_id uuid,
...
GRANT EXECUTE ON FUNCTION public.upsert_calendar_event_audit(
  uuid, uuid, uuid, text, date, date, text, text, text, text, uuid
) TO service_role;

-- Claims work atomically. SKIP LOCKED lets overlapping cron invocations share
-- the queue without sending the same PII to the model twice.
CREATE OR REPLACE FUNCTION public.claim_email_reconciliation_batch(p_limit integer DEFAULT 20)
RETURNS SETOF public.raw_emails
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = pg_catalog, public
AS $$
...
REVOKE ALL ON FUNCTION public.claim_email_reconciliation_batch(integer)
  FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.claim_email_reconciliation_batch(integer)
  TO service_role;

-- Final consolidation is one transaction and one lock domain. It adopts a
-- legacy exact-date reservation when present, otherwise creates the aggregate.
-- Every relation is tenant-scoped and every consumed input is unique.
CREATE OR REPLACE FUNCTION public.reconcile_email_extraction(
  p_extraction_id uuid,
  p_event_id uuid,
  p_confirmed_by_host boolean DEFAULT false
)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
REVOKE ALL ON FUNCTION public.reconcile_email_extraction(uuid, uuid, boolean)
  FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.reconcile_email_extraction(uuid, uuid, boolean)
  TO service_role;
