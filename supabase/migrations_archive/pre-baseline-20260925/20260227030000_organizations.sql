...
-- ============================================================
-- Adicionar organization_id a user_profiles FIRST (needed by helper function)
-- ============================================================
...
-- Skip if guests table doesn't exist yet (created in later migration)
-- ============================================================
DO $$ BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'guests') THEN
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'guests_email_org_unique') THEN
      ALTER TABLE public.guests ADD CONSTRAINT guests_email_org_unique UNIQUE (email, organization_id);
    END IF;
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'guests_select' AND tablename = 'guests') THEN
      CREATE POLICY "guests_select" ON public.guests FOR SELECT USING (organization_id = get_user_organization_id());
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'guests_insert' AND tablename = 'guests') THEN
      CREATE POLICY "guests_insert" ON public.guests FOR INSERT TO authenticated WITH CHECK (organization_id = get_user_organization_id());
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'guests_update' AND tablename = 'guests') THEN
      CREATE POLICY "guests_update" ON public.guests FOR UPDATE TO authenticated USING (organization_id = get_user_organization_id());
    END IF;
  END IF;
END $$;
