...
IF strpos(d,'AND rr.allow_declared_owner_base IS TRUE FOR SHARE;') > 0 THEN
  EXECUTE replace(d,'AND rr.allow_declared_owner_base IS TRUE FOR SHARE;','AND rr.allow_declared_owner_base IS TRUE;');
ELSIF strpos(d,'AND rr.allow_declared_owner_base IS TRUE;') = 0 THEN
