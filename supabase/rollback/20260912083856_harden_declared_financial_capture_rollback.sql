...
-- Explicitly repeatable: restore the captured definition; absent trigger/function is expected on rerun.
-- Keep the fail-closed NULL-role check: operational rollback must not reopen authorization.
DROP TRIGGER IF EXISTS reservations_protect_declared_financial_total ON public.reservations;
DROP FUNCTION IF EXISTS lodgra_private.protect_declared_reservation_total();
