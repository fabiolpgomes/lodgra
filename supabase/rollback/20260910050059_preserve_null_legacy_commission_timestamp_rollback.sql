...
-- Semantic data correction is intentionally monotonic: restoring the former
-- default/trigger would fabricate commission calculation timestamps.
ALTER TABLE public.reservations
  ALTER COLUMN commission_calculated_at DROP DEFAULT;
...
BEGIN
  ...
  ELSE
    NEW.external_reservation_id := COALESCE(NEW.external_id, NEW.external_reservation_id, NEW.id::text);
    NEW.external_id := NEW.external_reservation_id;
  END IF;
...
COMMENT ON COLUMN public.reservations.commission_calculated_at IS
  'Legacy timestamp. NULL means no legacy commission calculation was recorded; v2 finance uses canonical snapshots.';
